...
def recognize_face(face):
    ...
    try:
        ...
        if len(recognition_results[0]) > 0:
            face_rec_conf = recognition_results[0]["distance"].values[0]  # Distance metric (lower = better)
    ...
    # Match folder name in the database
...
def save_to_excel():
    ...
    # Adjust column width after writing
...
def format_excel(excel_path, sheet_name):
    """Format the Excel sheet with bold headers, decimal precision, centered alignment, and no borders."""
    try:
        wb = load_workbook(excel_path)
        sheet = wb[sheet_name]
        ...
        for cell in sheet[1]:
            ...
            cell.border = no_border  # Remove borders

        # Format the data cells
        for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row):
            for cell in row:
                if isinstance(cell.value, float):  # Round float values to 2 decimal places
...
start_time = time.time()  # Track session start time
...
while cap.isOpened():
    ...
    for track in tracked_objects:
        ...
        bbox = track.to_ltrb()
        identity = track.det_class  # Use recognized identity
...
sh_name = save_to_excel()
format_excel(excel_path, sh_name)
cap.release()
