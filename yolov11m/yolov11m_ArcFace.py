...
while True:
    ...
    for result in results:
        for box, conf in zip(result.boxes.xyxy, result.boxes.conf):
            ...
            recognition_results = DeepFace.find(img_path=face, db_path=db_path, model_name="ArcFace", detector_backend='ssd',
                                                enforce_detection=False)
            if len(recognition_results[0]) > 0:
                face_rec_conf = recognition_results[0]["distance"].values[0]
                if face_rec_conf < 0.5:
            ...
            identity_confidence = identity + '  ' + str(confid)
            if identity == 'Unknown':
